...
def strength(cards):
    ...
    maximum = 0 if len(card_counts) == 0 else max(card_counts, key=card_counts.get)
    card_counts[maximum] = card_counts.get(maximum, 0) + num_jokers
    max_card_count = card_counts.get(maximum)
